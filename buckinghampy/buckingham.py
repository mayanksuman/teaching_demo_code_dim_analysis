...
def find_nondimensional_numbers(
    parameters: List[Parameter], basis_parameter=None
    ...
    basis_parameter : Parameter (Optional)
        This parameter, if provided, should be in parameters list. 
        This parameter will come in numerator of nondimensional 
        number with unity power.
    ...
    # Check basis_parameter should be in parameters list
    if basis_parameter:
        if basis_parameter not in parameters:
            raise ValueError("Basis parameter should be in parameter list.")
        else:
            parameters.remove(basis_parameter)
            parameters = [basis_parameter, *parameters]
    ...
    _nondimensional_basis = _integrify_basis(nullspace)
    
    if basis_parameter:
        nondimensional_basis = list()
        for nb in _nondimensional_basis:
            nondimensional_basis.append([n/(nb[0] if nb[0] else 1) for n in nb])
    else:
        nondimensional_basis = _nondimensional_basis
